'https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=10&page=1&sparkline=false', { mode: 'no-cors' }
.then((data)=>{
    // console.log(data);
    return data.json();
}).then((e)=>{
...
function getData(e){
    ...
    e.forEach(e => {
        ...
        if(e.price_change_percentage_24h>0){
            document.getElementsByClassName('per')[temp].style.color = 'green';
        }else{
